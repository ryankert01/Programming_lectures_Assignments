...
vector::vector( const vector &right )
   : myFirst(),
    ...
    myEnd()
{
    size_type size = right.myLast - right.myFirst;
    size_type Res = right.myEnd - right.myFirst;
    myFirst = new value_type[Res]();
...
vector& vector::operator=( const vector &right )
{
   if( this != &right ) // avoid self-assignment
   {
      ...
      for (size_t i = 0; i < rightSize; i++) myFirst[i] = right.myFirst[i];
...
void vector::resize( const size_type newSize )
{
   ...
   if( newSize > originalSize )
   {
      if( newSize > capacity() )
      {
         ...
         myFirst = new value_type[newCapacity]();
         for (size_t i = 0; i < originalSize; i++) myFirst[i] = tempFirst[i];
         delete[]tempFirst;
         myEnd = myFirst + newCapacity;
      }
