...
public:
   ...
   vector( const vector &right )
      : myData()
   {
       myData.myFirst = (right.size() == 0 ? nullptr : new value_type[right.capacity()]);
   ...
   vector& operator=( const vector &right )
   {
      if( this != &right ) // avoid self-assignment
      {
         ...
         for (const_iterator it = right.begin(); it != right.end(); it++, it2++)
             *it2 = *it;
         myData.myLast = it2;
...
void HugeInteger< T >::operator-=( HugeInteger< T > &op2 )
{
   ...
   for (size_type i = 0; i < op2.size(); i++)
       difference.integer[i] -= op2.integer[i];
   for (size_type i = 0; i < difference.size()-1; i++)
       if (difference.integer[i] < 0 || difference.integer[i] > 100)
       {
...
void HugeInteger< T >::operator*=( HugeInteger< T > &op2 )
{
   ...
   if( isZero() || op2.isZero() )
   {
    ...
    for (size_type i = 0; i < size(); i++)
        for (size_type j = 0; j < op2.size(); j++)
...
HugeInteger< T > HugeInteger< T >::operator/( HugeInteger< T > &op2 )
{
   ...
   HugeInteger< T >ten(2);
   ten.convert(10);
   size_type a = this->size() - op2.size();
   for (size_type i = 0; i < a; i++)
       buffer *= ten;
   HugeInteger< T > quotient(a);
   if (remainder < buffer ) {
       buffer.divideByTen();
   }
   else
      quotient.integer.resize(a+1);
